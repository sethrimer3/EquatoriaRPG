...
import type {
  ...
  EigensteinEnemy, BossEnemy, AlivenSwarmEnemy,
...
export function performWeaponAttack(ctx: RpgPlayerAttackCtx, weaponId: string): void {
  ...
  if (effect.kind === 'aoe') {
    ...
    for (const swarm of alivenedEnemies) {
      const dx = swarm.x - mote.x, dy = swarm.y - mote.y;
      if (dx * dx + dy * dy <= aoeRadius * aoeRadius) {
        const dmg = damageAlivenSwarmEnemy(swarm, rawDamage, 0);
        spawnHitVisualsAt(swarm.x, swarm.y, swarm.maxHp, dmg, '#cc88ff');
      }
    }
    if (bossEnemy) {
  ...
  if (effect.kind === 'multi') {
    ...
    for (const swarm of alivenedEnemies) {
      ...
      const dx = swarm.x - mote.x, dy = swarm.y - mote.y;
      const d = dx * dx + dy * dy;
      if (d <= rangeSq) inRange.push({ distSq: d, alivened: swarm });
    }
    if (bossEnemy) {
      const dx = bossEnemy.x - mote.x, dy = bossEnemy.y - mote.y;
      const d = dx * dx + dy * dy;
    ...
    for (const t of targets) {
      if (t.laser) {
        ...
        const dmg = damageAlivenSwarmEnemy(t.alivened, rawDamage, 0);
        spawnHitVisualsAt(t.alivened.x, t.alivened.y, t.alivened.maxHp, dmg, '#cc88ff');
      } else if (t.boss) {
        const dmg = damageBossEnemy(rawDamage, 0);
  ...
  } else if (closestT.alivened) {
    const dmg = damageAlivenSwarmEnemy(closestT.alivened, rawDamage, defPierceRatio);
    spawnHitVisualsAt(closestT.alivened.x, closestT.alivened.y, closestT.alivened.maxHp, dmg,
      effect.kind === 'piercing' ? '#74c0fc' : '#cc88ff');
