...
import type {
  ...
  NullstoneEnemy, FracterylEnemy, EigensteinEnemy, BossEnemy, AlivenSwarmEnemy,
...
export function updatePlayerMovement(
  ...
  if (speed > 0.1) {
    ...
    for (const swarm of ctx.alivenedEnemies) if (swarm.particles.length > 0) checkAimEnemy(swarm);
...
function _findNearestEnemy(ctx: PlayerMovementCtx): { distSq: number; x: number; y: number } {
  ...
  for (const swarm of ctx.alivenedEnemies) if (swarm.particles.length > 0) check(swarm);
